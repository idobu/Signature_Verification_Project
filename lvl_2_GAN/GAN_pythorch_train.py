...
def train_GAN_Model():
    ...
    for epoch in range(GAN_Configuration.num_epochs):
        ...
        if epoch>=20:
            ...
            if abs(curr - GAN_Configuration.Desired_loss_threshold) <= 1 and (epoch >= GAN_Configuration.num_epochs*GAN_Configuration.Percent_of_all_dataset_stop) and D_x > 0.9:
                print(f"Losses are around {GAN_Configuration.Desired_loss_threshold} for the Generator. Stopping training.")
                break
    ...
    ani.save('detailed_animation.gif', writer='pillow', fps=3)  # Adjust 'fps' and 'interval' as needed
